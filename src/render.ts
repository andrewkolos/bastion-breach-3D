...
export class Stage {
  ...
  public resetGame() {
    const disposeDeck = (deck: PhysicalDeck) => {
  ...
  private mouseup = () => {
    ...
    if (this.playing && this.readyToPlay && playerObject) {
      ...
      const neutralEntries = this.neutralBoard.objects();
      const currentNeutralObject = neutralEntries[this.playerBoard.size()];

      new TweenGroup(
        this.moveCard(
          playerObject,
          playerObject.position.clone().add(new THREE.Vector3(0, 0.7, -1)),
        ...
        () => {
          new TweenGroup(
            this.moveCard(
              playerObject,
              currentNeutralObject.position.clone().add(new THREE.Vector3(0, 0, 726 / 500 + 0.02)),
            ...
            () => {
              new TweenGroup(
                this.moveCard(
                  playerObject,
                  playerObject.position.clone().add(new THREE.Vector3(0, 1, 0)),
      ...
      new TweenGroup(
        this.dealCards(
          Array.from(this.playerHand.objects()),
          new THREE.Vector3(0, 0.4, 3),
          new THREE.Vector3(-0.2, 0.01, 0),
          new THREE.Euler(-Math.PI / 2.8, 0, 0),
          300,
          1,
        ),
      ).start();
      ...
      const opponentObject = this.computerHand.objects()[Math.floor(Math.random() * this.computerHand.size())]; // get random opponent card

      new TweenGroup(
        this.moveCard(
          opponentObject,
          opponentObject.position.clone().add(new THREE.Vector3(0, 0.5, 0)),
          new THREE.Euler(Math.PI / 2, 0, 0),
          200,
          0,
        ),
          ...
          new TweenGroup(
            ...
            () => {
              ...
              new TweenGroup(
                this.moveCard(
                  opponentObject,
                  opponentObject.position.clone().add(new THREE.Vector3(0, 1, 0)),
                  new THREE.Euler(-Math.PI / 2),
                  200,
                  200,
                ),
      ...
      new TweenGroup(
        this.dealCards(
          Array.from(this.computerHand.objects()),
        ...
        new TWEEN.Tween(opacity)
          ...
          .onUpdate(function () {
            if (object instanceof THREE.Mesh) {
              // circle
              (object.material as THREE.Material).opacity = opacity.value;
            }
            object.children.forEach((c: THREE.Object3D) => {
              // square or cross
              (c as any).material.opacity = opacity.value;
            });
          })
          .start();
      };
      if (winner === playerCard) {
        this.indicatorTimeouts.push(
          setTimeout(() => {
  ...
  handleRaycasting = () => {
    const vector = new THREE.Vector3(this.mouseVector.x, this.mouseVector.y, 1);
  ...
  private initGround() {
    ...
    const material = new THREE.MeshPhongMaterial({ map: this.resources.grassTexture });
  ...
  private createCards(boardSuit: SUIT, playerSuit: SUIT, computerSuit: SUIT) {
    ...
    new TweenGroup(
      this.dealCards(
        Array.from(this.neutralBoard.objects()),
  ...
  private moveCard(
    ...
    const targetPosition = { x: pos.x, y: pos.y, z: pos.z };

    const tween1 = new TWEEN.Tween(position)
      .to(targetPosition, posDuration)
      .easing(TWEEN.Easing.Quadratic.Out)
      .onUpdate(function () {
        object.position.set(position.x, position.y, position.z);
      });
    const rotation = { x: object.rotation.x, y: object.rotation.y, z: object.rotation.y };
    const targetRotation = { x: rot.x, y: rot.y, z: rot.z };
    const tween2 = new TWEEN.Tween(rotation)
      .to(targetRotation, rotDuration)
      .easing(TWEEN.Easing.Quadratic.Out)
      .onUpdate(function () {
